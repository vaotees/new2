...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "GET") {
    try {
      let config = await prisma.sectionFeaturesConfig.findUnique({
        where: { id: "singleton" }
      })

      if (!config) {
        config = await prisma.sectionFeaturesConfig.create({
          data: { id: "singleton" }
        })
      }
  ...
  if (req.method === "PUT") {
    ...
    try {
      ...
      const config = await prisma.sectionFeaturesConfig.upsert({
        where: { id: "singleton" },
